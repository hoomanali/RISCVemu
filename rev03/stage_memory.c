...
#define OFFSET_MASK_D 0x7
#define INDEX_MASK_D 0x3FF8
#define TAG_MASK_D 0xFFFFFFFFFFFFC000
    ...
    if (class == lb || class == lh || class == lw || class == ld || class == ld || class == lbu) {
        ...
        for (x = 0; x < 2048; x++) {
            if (d_cache[x].valid_bit == true) {
                if (d_cache[x].index == (rs1_val+imm) && INDEX_MASK_D) {
                    if (d_cache[x].tag == (rs1_val+imm) && TAG_MASK_D) {
        ...
        if (current_stage_w_register->memoryread == 1) {
            if (memory_status((rs1_val+imm), &value)) {
                for (x = 0; x < 2048; x++) {
                    if (d_cache[x].valid_bit == false) {
                        d_cache[x].valid_bit = true;
                        d_cache[x].data = value;
                        d_cache[x].offset = (rs1_val+imm) && OFFSET_MASK_D;
                        d_cache[x].index = (rs1_val+imm) && INDEX_MASK_D;
                        d_cache[x].tag = (rs1_val+imm) && TAG_MASK_D;
            ...
            switch(class) {
                case lb:    // LB
                    if (memory_read((rs1_val+imm), &value, 1)) {
                        for (x = 0; x < 2048; x++) {
                            if (d_cache[x].valid_bit == false) {
                                d_cache[x].valid_bit = true;
                                d_cache[x].data = value;
                ...
                case lh:    // LH
                    if (memory_read((rs1_val+imm), &value, 2)) {
                        for (x = 0; x < 2048; x++) {
                            if (d_cache[x].valid_bit == false) {
                                d_cache[x].valid_bit = true;
                                d_cache[x].data = value;
                ...
                case lw:    // LW
                    if (memory_read((rs1_val+imm), &value, 4)) {
                        for (x = 0; x < 2048; x++) {
                            if (d_cache[x].valid_bit == false) {
                                d_cache[x].valid_bit = true;
                                d_cache[x].data = value;
                ...
                case ld:    // LD
                    if (memory_read((rs1_val+imm), &value, 8)) {
                        for (x = 0; x < 2048; x++) {
                            if (d_cache[x].valid_bit == false) {
                                d_cache[x].valid_bit = true;
                                d_cache[x].data = value;
